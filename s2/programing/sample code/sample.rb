...
def read_tracks(file)
    ...
    while i < count
        ...
        track_duration = file.gets.chomp      # Read track duration
...
def read_album(file)
    ...
    return Album.new(artist, title, label, genre, tracks) #creating a new album object 
end
...
def read_albums()
    ...
    albums = []
    file = File.new(file_path, 'r') # Open a file in read mode using the file path provided in 'file_path'
    count = file.gets.to_i

    i = 0
    while i < count
        albums << read_album(file) #calling method  #<< element is added to array in the end  
        i += 1
    end

    file.close
    return albums #ensure file exists and then it reads all albums in the file and returns an array of album objects 
end
...
def display_albums(albums)
    ...
    i = 0
    while i < albums.length #artist, genre array
        album = albums[i]
...
def select_track_menu(album)
    ...
    while i < album.tracks.length
        puts "#{i}. #{album.tracks[i].name.ljust(20)} #{album.tracks[i].location.ljust(40)} Duration: #{album.tracks[i].duration}"
        i += 1
    end
